...
CSV_HEADERS = ["Class", "Sample Number", "Wh", "IR", "UV", "730nm", "760nm", "810nm", "860nm", "900nm", "940nm"]
...
class CornGUI:
    def __init__(self, root, csv_file="corn_data.csv"):
        ...
        self.rows = []  # list of tuples: (classification, sample number, white, ir, uv, 730, 760, 810, 860, 900, 940)
    ...
    def _rebuild_table(self, force_last=False):
        ...
        for r_index, row in enumerate(rows_to_display, start=1):
            classification, sample_number, white, ir, uv, t, u, v, wv, k, l = row
            row_no = start + r_index  # 1-based overall row number
            display_values = [row_no, classification, sample_number, white, ir, uv, t, u, v, wv, k, l]
    ...
    def _load_existing_csv(self):
        ...
        try:
            with open(self.csv_file, "r", newline="", encoding="utf-8") as f:
                ...
                sample_number_idx = index_of_any(["Sample Number", "Sample"], header)
                idx_map.append(sample_number_idx)
                ...
                for row in r:
                    try:
                        ...
                        white = int(float(get_val(idx_map[2], cast=float, default=0))) if idx_map[2] is not None else 0
                        ir = int(float(get_val(idx_map[3], cast=float, default=0))) if idx_map[3] is not None else 0
                        uv = int(float(get_val(idx_map[4], cast=float, default=0))) if idx_map[4] is not None else 0
                        t = float(get_val(idx_map[5], cast=float, default=0.0))
                        u = float(get_val(idx_map[6], cast=float, default=0.0))
                        v = float(get_val(idx_map[7], cast=float, default=0.0))
                        wv = float(get_val(idx_map[8], cast=float, default=0.0))
                        k = float(get_val(idx_map[9], cast=float, default=0.0))
                        l = float(get_val(idx_map[10], cast=float, default=0.0))
                        sample_number = get_val(idx_map[1], cast=None, default="")
                        loaded.append((classification, sample_number, white, ir, uv, t, u, v, wv, k, l))
                    except Exception:
                        # Skip malformed rows
                        continue
                # Keep chronological order in memory (oldest->newest) and do not trim CSV
                self.rows = loaded
                if sample_number_idx is None:
                    self._atomic_write_rows(self.rows)
        except OSError as e:
    ...
    def _ask_sample_number(self):
        dialog = tk.Toplevel(self.root)
        dialog.title("Sample Number")
        dialog.transient(self.root)
        dialog.grab_set()
        dialog.resizable(False, False)

        # Keep the keypad well inside the screen for difficult-to-press edges.
        ...
        margin = 18
        dialog_height = min(dialog_height, max(360, root_height - (margin * 2)))
        dialog_width = min(dialog_width, max(300, root_width - (margin * 2)))
        x = root_x + max(margin, (root_width - dialog_width) // 2)
        y = root_y + max(margin, (root_height - dialog_height) // 2)
        ...
        ttk.Label(content, text="Enter sample number", font=("TkDefaultFont", 14, "bold")).pack(pady=(0, 8))
        entry = ttk.Entry(content, textvariable=sample_number, justify="center", font=("TkDefaultFont", 22), state="readonly")
        entry.pack(fill="x", ipady=5, pady=(0, 10))
        ...
        def append_digit(digit):
            sample_number.set(sample_number.get() + digit)

        def clear_number():
            sample_number.set("")

        def remove_digit():
            sample_number.set(sample_number.get()[:-1])

        def confirm():
            value = sample_number.get().strip()
            if value:
                result["value"] = value
                dialog.destroy()

        for index, digit in enumerate(("1", "2", "3", "4", "5", "6", "7", "8", "9")):
            ttk.Button(keypad, text=digit, command=lambda value=digit: append_digit(value)).grid(
                row=index // 3, column=index % 3, sticky="nsew", padx=4, pady=4, ipadx=6, ipady=6
            )
        ttk.Button(keypad, text="Clear", command=clear_number).grid(row=3, column=0, sticky="nsew", padx=4, pady=4, ipadx=6, ipady=6)
        ttk.Button(keypad, text="0", command=lambda: append_digit("0")).grid(row=3, column=1, sticky="nsew", padx=4, pady=4, ipadx=6, ipady=6)
        ttk.Button(keypad, text="Backspace", command=remove_digit).grid(row=3, column=2, sticky="nsew", padx=4, pady=4, ipadx=6, ipady=6)

        actions = ttk.Frame(content)
        actions.pack(fill="x", pady=(10, 0))
        actions.grid_columnconfigure(0, weight=1)
        actions.grid_columnconfigure(1, weight=1)
        ttk.Button(actions, text="Cancel", command=dialog.destroy).grid(row=0, column=0, sticky="ew", padx=(0, 4), ipady=5)
        ttk.Button(actions, text="Confirm", command=confirm).grid(row=0, column=1, sticky="ew", padx=(4, 0), ipady=5)
    ...
    def _finish_capture_success(self, row):
        ...
        sample_number = self._ask_sample_number()
        if sample_number is None:
            self.capture_progress.set(0)
            self.capture_in_progress = False
            self._set_buttons_state("normal")
            self.status_var.set("Measurement discarded: sample number required.")
            return

        row = (classification, sample_number.strip(), white, ir, uv, t, u, v, wv, k, l)
        self.rows.append(row)
        self._append_csv(row)
        self._rebuild_table(force_last=True)
        self.capture_progress.set(0)
        self.capture_in_progress = False
        self.status_var.set("Sample captured.")
        self._set_buttons_state("normal")
